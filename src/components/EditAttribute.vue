      ...
      <div name="1" v-show="!editMode">
        <div>
          <div class="item" v-for="(item, index) in localAttributes" :key="index">
            <el-input v-model="item.key" :placeholder="'key' + index" class="halfWidth"></el-input>
            <div class="split">:</div>
            <el-input v-model="item.value" :placeholder="'value' + index" class="halfWidth" style="flex-grow: 3;"></el-input>
          ...
          <div class="quickAddRoot">
            ...
            <div style="margin-top: 5px;">
              <transition name="el-zoom-in-center">
                <el-tag v-if="attributeKeys.indexOf('class') == -1" size="small" type="success" @click="onClassClick" effect="dark" class="tag">Class
                </el-tag>
              </transition>
              <transition name="el-zoom-in-center">
                <el-tag v-if="attributeKeys.indexOf('@click') == -1" size="small" type="success" @click="onEventClick" effect="dark" class="tag">点击事件</el-tag>
              </transition>
              <transition name="el-zoom-in-center">
                <el-tag v-if="!attributeKeys.includes('__text__')" size="small" type="success" @click="onTextClick" effect="dark" class="tag">文本内容</el-tag>
              </transition>
            </div>
          </div>
        </div>
